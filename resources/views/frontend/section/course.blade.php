    ...
    <div class="container">
        ...
        <ul class="nav nav-tabs generic-tab justify-content-center pb-4" id="myTab" role="tablist">
           @foreach($all_categories as $key => $value)
             <li class="nav-item">
               <a class="nav-link {{ $key == 0 ? 'active' : '' }}" id="{{ $value->slug }}-tab" data-toggle="tab" href="#{{ $value->slug }}" role="tab" aria-controls="{{ $value->slug }}" aria-selected="{{ $key == 0 ? 'true' : 'false' }}">{{ $value->name }}</a>
            </li>
           @endforeach
        ...
        <div class="container">
            <div class="tab-content" id="myTabContent">
                @foreach ($all_categories as $key => $data )
                    <div class="tab-pane fade show {{ $key == 0 ? ' active' : '' }}" id="{{ $data->slug }}" role="tabpanel" aria-labelledby="{{ $data->slug }}-tab">
                    <div class="row">
                            ...
                            <div class="card card-item card-preview" data-tooltip-content="#{{ $course->course_name_slug }}">
                                <div class="card-image">
                                    <a href="{{ route('course.details', $course->course_name_slug) }}" class="d-block">
                                        <img class="card-img-top lazy" width="240" height="240" src="{{ asset($course->course_image) }}" data-src="{{ asset($course->image) }}" alt="Card image cap">
                                    </a>
